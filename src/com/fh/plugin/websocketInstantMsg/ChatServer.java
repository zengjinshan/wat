...
public class ChatServer extends WebSocketServer{
	...
	public void onMessage(WebSocket conn, String message){
		...
		if(null != message && message.startsWith("success")){
			this.userjoin(message.replaceFirst("success", ""),conn);
		}if(null != message && message.startsWith("LeaveSuccess")){
			this.userLeave(conn);
		}if(null != message && message.contains("toUser")){
			String toUser = message.substring(message.indexOf("toUser")+10, message.indexOf("admin888"));
			message = message.substring(0, message.indexOf("toUser")) +"[私信]  "+ message.substring(message.indexOf("admin888")+12, message.length());
